...
unsigned int ia32_insn_implicit_ops( x86_insn_t *insn, unsigned int impl_idx ) {
	...
	for ( list = op_implicit_list[impl_idx]; list->type; list++, num++ ) {
		enum x86_op_access access = static_cast<enum x86_op_access>( OP_PERM(list->type) );
		enum x86_op_flags  flags  = static_cast<enum x86_op_flags>(OP_FLAGS(list->type) >> 12);
		...
		op->access |= static_cast<x86_op_access>(access);
		op->flags |= static_cast<x86_op_flags>(flags);
		op->flags |= static_cast<x86_op_flags>(op_implied);
	}
